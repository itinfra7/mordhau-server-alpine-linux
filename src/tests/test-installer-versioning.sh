...
for version in 0.0.0 2.5.0 10.20.300; do
...
[ "$(compare_versions 2.4.0 2.5.0)" -eq -1 ]
[ "$(compare_versions 2.5.0 2.5.0)" -eq 0 ]
[ "$(compare_versions 2.6.0 2.5.0)" -eq 1 ]
[ "$(compare_versions 3.0.0 2.99.99)" -eq 1 ]

prepare_version_transition > "$TEST_ROOT/fresh.log"
grep -Fq 'Installing mordhau-server-alpine-linux 2.5.0.' "$TEST_ROOT/fresh.log"
[ -z "$INSTALLED_VERSION" ]

printf '%s\n' 2.4.0 > "$STATE_DIR/manager-version"
prepare_version_transition > "$TEST_ROOT/upgrade.log"
grep -Fq \
    'Upgrading mordhau-server-alpine-linux 2.4.0 -> 2.5.0.' \
...
grep -Fq \
    'Reinstalling mordhau-server-alpine-linux 2.5.0 for validation.' \
...
grep -Fq \
    'Downgrading mordhau-server-alpine-linux 2.6.0 -> 2.5.0.' \
...
[ "$(cat "$STATE_DIR/manager-version")" = 2.5.0 ]
