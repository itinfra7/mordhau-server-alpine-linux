...
for version in 0.0.0 2.4.0 10.20.300; do
...
[ "$(compare_versions 2.3.3 2.4.0)" -eq -1 ]
[ "$(compare_versions 2.4.0 2.4.0)" -eq 0 ]
[ "$(compare_versions 2.5.0 2.4.0)" -eq 1 ]
[ "$(compare_versions 3.0.0 2.99.99)" -eq 1 ]

prepare_version_transition > "$TEST_ROOT/fresh.log"
grep -Fq 'Installing mordhau-server-alpine-linux 2.4.0.' "$TEST_ROOT/fresh.log"
[ -z "$INSTALLED_VERSION" ]

printf '%s\n' 2.3.3 > "$STATE_DIR/manager-version"
prepare_version_transition > "$TEST_ROOT/upgrade.log"
grep -Fq \
    'Upgrading mordhau-server-alpine-linux 2.3.3 -> 2.4.0.' \
    "$TEST_ROOT/upgrade.log"
[ "$INSTALLED_VERSION" = 2.3.3 ]

printf '%s\n' 2.4.0 > "$STATE_DIR/manager-version"
prepare_version_transition > "$TEST_ROOT/reinstall.log"
grep -Fq \
    'Reinstalling mordhau-server-alpine-linux 2.4.0 for validation.' \
    "$TEST_ROOT/reinstall.log"

printf '%s\n' 2.5.0 > "$STATE_DIR/manager-version"
if (
    ALLOW_DOWNGRADE=0
...
grep -Fq \
    'Downgrading mordhau-server-alpine-linux 2.5.0 -> 2.4.0.' \
...
[ "$(cat "$STATE_DIR/manager-version")" = 2.4.0 ]
