...
for version in 0.0.0 2.6.2 10.20.300; do
...
[ "$(compare_versions 2.6.1 2.6.2)" -eq -1 ]
[ "$(compare_versions 2.6.2 2.6.2)" -eq 0 ]
[ "$(compare_versions 2.7.0 2.6.2)" -eq 1 ]
[ "$(compare_versions 3.0.0 2.99.99)" -eq 1 ]

prepare_version_transition > "$TEST_ROOT/fresh.log"
grep -Fq 'Installing mordhau-server-alpine-linux 2.6.2.' "$TEST_ROOT/fresh.log"
[ -z "$INSTALLED_VERSION" ]

printf '%s\n' 2.6.1 > "$STATE_DIR/manager-version"
prepare_version_transition > "$TEST_ROOT/upgrade.log"
grep -Fq \
    'Upgrading mordhau-server-alpine-linux 2.6.1 -> 2.6.2.' \
    "$TEST_ROOT/upgrade.log"
[ "$INSTALLED_VERSION" = 2.6.1 ]

printf '%s\n' 2.6.2 > "$STATE_DIR/manager-version"
prepare_version_transition > "$TEST_ROOT/reinstall.log"
grep -Fq \
    'Reinstalling mordhau-server-alpine-linux 2.6.2 for validation.' \
...
grep -Fq \
    'Downgrading mordhau-server-alpine-linux 2.7.0 -> 2.6.2.' \
...
[ "$(cat "$STATE_DIR/manager-version")" = 2.6.2 ]
