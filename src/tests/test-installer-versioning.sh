...
for version in 0.0.0 2.6.5 10.20.300; do
...
[ "$(compare_versions 2.6.4 2.6.5)" -eq -1 ]
[ "$(compare_versions 2.6.5 2.6.5)" -eq 0 ]
[ "$(compare_versions 2.7.0 2.6.5)" -eq 1 ]
[ "$(compare_versions 3.0.0 2.99.99)" -eq 1 ]

prepare_version_transition > "$TEST_ROOT/fresh.log"
grep -Fq 'Installing mordhau-server-alpine-linux 2.6.5.' "$TEST_ROOT/fresh.log"
[ -z "$INSTALLED_VERSION" ]

printf '%s\n' 2.6.4 > "$STATE_DIR/manager-version"
prepare_version_transition > "$TEST_ROOT/upgrade.log"
grep -Fq \
    'Upgrading mordhau-server-alpine-linux 2.6.4 -> 2.6.5.' \
...
grep -Fq \
    'Reinstalling mordhau-server-alpine-linux 2.6.5 for validation.' \
...
grep -Fq \
    'Downgrading mordhau-server-alpine-linux 2.7.0 -> 2.6.5.' \
...
[ "$(cat "$STATE_DIR/manager-version")" = 2.6.5 ]
