...
func TestDashboardThemeAndServerPromptMarkup(t *testing.T) {
	...
	for _, expected := range []string{
		...
		`src="/static/theme.js?v=2.6.9"`,
		`href="/static/app.css?v=2.6.9"`,
		`src="/static/app.js?v=2.6.9"`,
		`<body id="page-top">`,
		...
		`id="panel-players"`,
		`id="player-search"`,
		`placeholder="Search PlayFabID or nickname"`,
		`id="player-sort-field"`,
		`id="player-sort-direction"`,
		`id="player-geoip-status"`,
	...
	if !strings.Contains(loginSource, `src="/static/theme.js?v=2.6.9"`) {
		t.Fatal("login page does not initialize the persisted theme")
	}
	if !strings.Contains(loginSource, `href="/static/app.css?v=2.6.9"`) {
...
func TestMobileLayoutHasTouchAndNarrowViewportRules(t *testing.T) {
	...
	for _, expected := range []string{
		...
		`.player-country-flag`,
		`.player-address-location`,
		`.player-moderation-grid`,
